...
export function Carrusel({ movies, title = "Películas en Tendencia", autoPlayMs = 0 }: Props) {
  ...
  useEffect(() => {
    ...
    setIsTouch(hasTouch);
    console.log("[Carrusel] Touch device detected:", hasTouch);
  }, []);
      ...
      <div className="relative overflow-hidden rounded-lg border-2 border-red-500 bg-gray-800">
        <div className="relative h-110">
          ...
          <div
            ...
            onPointerDown={(e) => {
              // ignore pointer events generated by touch (touch handled above)
              if ((e as React.PointerEvent).pointerType === "touch") return;
              pointerId.current = (e as React.PointerEvent).pointerId;
              pointerStartX.current = (e as React.PointerEvent).clientX;
              try {
                (e.target as Element).setPointerCapture?.((e as React.PointerEvent).pointerId);
              } catch (err) {
                // ignore
              }
            }}
            onPointerMove={(e) => {
              // optional: we could implement drag UI here
            }}
            onPointerUp={(e) => {
              if ((e as React.PointerEvent).pointerId !== pointerId.current) return;
              const endX = (e as React.PointerEvent).clientX;
          ...
          >
            {showArrows && (
              <>
                <button
                  type="button"
                  onClick={() => { prev(); setShowArrows(false); }}
                ...
                <button
                  type="button"
                  onClick={() => { next(); setShowArrows(false); }}
